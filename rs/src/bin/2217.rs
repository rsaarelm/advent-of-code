...
fn in_well(rock: &HashSet<IVec2>) -> bool {
...
fn main() {
    ...
    let rocks: Vec<HashSet<IVec2>> = ROCKS.split("\n\n").map(points).collect();

    let mut ground = HashSet::default();
    let mut top = 0;
    let mut wind_idx = 0;
    ...
    let mut states: HashMap<(Vec<i32>, usize, usize), usize> =
        Default::default();
